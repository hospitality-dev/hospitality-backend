#set text(font: "Lato")
#let warning(title, hasAboutToExpire) = table.cell(
  fill: if hasAboutToExpire == true { luma(150) } else { luma(250) },
  text(fill: black, title, weight: if hasAboutToExpire == true { "semibold" } else { "regular" }, size: 12pt),
)
...
#show: doc => inventory-list(title: sys.inputs.at("location_title"))
